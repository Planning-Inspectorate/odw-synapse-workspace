...
def test_upload_new_wheel__with_package_already_bound_to_spark_pool():
    """
        Given the new odw package already exists in the workspace, and is bound to the [pinssynspodwpr or pinssynspodw35] spark pools
        When i try to upload a new package
...
def test_upload_new_wheel__with_existing_odw_package_already_bound_to_external_spark_pool():
    """
        Given there is already an odw package in the workspace bound to a pool other than [pinssynspodwpr or pinssynspodw35]
        When i try to upload a new odw package
        Then the [pinssynspodwpr or pinssynspodw35] pools should be updated to use the new package, but the old package must be left as-is
    """
    env = "mock_env"
