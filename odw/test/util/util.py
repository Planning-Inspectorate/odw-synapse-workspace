...
def get_all_files_in_directory(inst, source_path: str):
    ...
    while folders:
        ...
        new_files = [
            ...
            for x in subfolder_contents
            if os.path.isfile(os.path.join(subfolder, x))
        ]
        files += new_files
        new_folders = [
            os.path.join(subfolder, x)
            for x in subfolder_contents
            if os.path.isdir(os.path.join(subfolder, x))
        ]
        folders += new_folders
    return files
...
def format_adls_path_to_local_path(
    ...
    """
    return os.path.join(
        PytestSparkSessionUtil().get_spark_warehouse_name(), container_name, blob_path
    )
