...
class TestServiceBusStandardisationProcess(SparkTestCase):
    def test__service_bus_standardisation_process__get_max_file_date(self):
    ...
    def test__service_bus_standardisation_process__remove_data_duplicates(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df = spark.createDataFrame(
            [
