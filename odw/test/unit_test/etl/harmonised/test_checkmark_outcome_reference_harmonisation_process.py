...
from odw.core.etl.transformation.harmonised.checkmark_outcome_reference_harmonisation_process import (
...
_SAMPLE_ROW_FULL = ("id-001", "Allowed")

_SOURCE_COLUMNS = ["ID", "value"]

_MODULE = "odw.core.etl.transformation.harmonised.checkmark_outcome_reference_harmonisation_process"
...
class TestCheckmarkOutcomeReferenceHarmonisationProcess(SparkTestCase):
    ...
    def test__process__rowid_changes_when_value_changes(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_a = _build_input_df(spark, [("id-001", "Allowed")])
        df_b = _build_input_df(spark, [("id-001", "Dismissed")])
    ...
    def test__process__data_to_write_has_overwrite_mode(self):
        ...
        assert write_metadata["blob_path"] == "outcome_reference"

    def test__process__returns_success_result_with_insert_count(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _build_input_df(spark, [("id-001", "Allowed"), ("id-002", "Dismissed")])
