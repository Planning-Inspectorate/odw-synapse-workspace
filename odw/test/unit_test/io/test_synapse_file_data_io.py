...
class TestSynapseFileDataIO(SparkTestCase):
    def test__synapse_file_data_io__read__successful(self, tmpdir):
        ...
        with mock.patch.object(
            SynapseFileDataIO, "_format_to_adls_path", return_value=mock_file_path
        ):
    ...
    @pytest.mark.parametrize(
        "argument_to_drop",
        ["storage_name", "container_name", "blob_path", "file_format", "spark"],
    )
    def test__synapse_file_data_io__read__with_missing_arguments(
    ...
    def test__synapse_file_data_io__write__successful(self, tmpdir):
        spark_session = PytestSparkSessionUtil().get_spark_session()
        mock_dataframe: DataFrame = spark_session.createDataFrame(
            [(1, "a"), (2, "b"), (3, "c")],
            StructType(
    ...
    @pytest.mark.parametrize(
        "argument_to_drop",
        ["storage_name", "container_name", "blob_path", "file_format", "write_mode"],
    )
    def test__synapse_file_data_io__write__with_missing_arguments(
        self, argument_to_drop: str
    ):
        spark_session = PytestSparkSessionUtil().get_spark_session()
        mock_dataframe: DataFrame = spark_session.createDataFrame(
