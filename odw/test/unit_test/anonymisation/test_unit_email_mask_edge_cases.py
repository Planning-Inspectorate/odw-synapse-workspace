...
def test__email_mask_strategy__handles_edge_cases():
    ...
    assert valid_email_row["email"] == "j******e@pins.com", "Valid email should be masked with @pins.com domain"

    # Minimal valid email
    minimal_email_row = [r for r in results if r["description"] == "Minimal valid email (single char local)"][0]
    assert minimal_email_row["email"] == "a@pins.com", "Single character local part should be preserved with @pins.com"
    ...
    assert no_domain_row["email"] == "a**@", "'abc@' should be treated as non-email (@ preserved in mask)"
    assert "@pins.com" not in no_domain_row["email"], "'abc@' should NOT be treated as a valid email"

    no_local_row = [r for r in results if r["description"] == "Invalid: no local part"][0]
    # FIXED: "@domain.com" is now treated as non-email -> "@*********m"
    assert no_local_row["email"] == "@*********m", "'@domain.com' should be treated as non-email"
    assert "@pins.com" not in no_local_row["email"], "'@domain.com' should NOT be treated as a valid email"
    ...
    assert text_with_at_row["email"] == "n******************e", "Text with @ should be treated as non-email"
    assert "@pins.com" not in text_with_at_row["email"], "Text with @ should NOT be treated as a valid email"
    ...
    assert "@pins.com" not in double_dot_row["email"], "'user@example..com' should NOT be treated as a valid email (consecutive dots)"
    assert double_dot_row["email"] == "u***************m", "'user@example..com' should be masked as non-email"


def test__email_mask_strategy__with_improved_validation():
    """
    Test that EmailMaskStrategy properly validates emails using regex
    before applying email-specific masking.

    This verifies the improved implementation correctly handles edge cases.
    """
    spark = PytestSparkSessionUtil().get_spark_session()

    test_data = [
        ("john.doe@example.com", "j******e@pins.com"),  # Valid email
        ("a@x.com", "a@pins.com"),  # Valid minimal email
        ("abc@", "a**@"),  # Invalid: no domain - masked as non-email (@ preserved)
        ("@domain.com", "@*********m"),  # Invalid: no local part - masked as non-email
        ("not an email @ value", "n******************e"),  # Invalid: text with @ - masked as non-email
        ("Price @ £100", "P**********0"),  # Invalid: @ as 'at' symbol - masked as non-email
