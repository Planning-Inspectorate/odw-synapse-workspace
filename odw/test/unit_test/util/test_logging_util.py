...
def test_logging_util_is_a_singleton():
    with mock.patch.object(LoggingUtil, "_initialise", return_value=None):
        ...
        assert id(instance_a) == id(instance_b), (
            "Expected the same object to be returned by the constructor, but different objects were returned"
        )
...
def test_logging_util__initialise():
    ...
    with mock.patch(
        ...
        with mock.patch.object(
            ...
            "getSecretWithLS",
            return_value="some_connection_string;blah;blah",
        ):
            with mock.patch.object(LoggingUtil, "setup_logging"):
...
def test_logging_util__log_error():
    ...
    with mock.patch.object(logging.Logger, "error", return_value=None):
        ...
        logging.Logger.error.assert_called_once_with(
            f"{pipeline_guid} : {error_message}"
        )
...
def test_logging_util__log_exception():
    ...
    with mock.patch.object(logging.Logger, "exception", return_value=None):
        ...
        logging.Logger.exception.assert_called_once_with(
            f"{pipeline_guid} : {exception_message}"
        )
...
@pytest.mark.parametrize(
    ...
    [
        (
            False,
            False,
        ),  # Without force initialisation. i.e. If not initialised, then initialise
        (
            True,
            True,
        ),  # With forced initialisation. i.e. If already initialised and running with force, then initialise
    ],
)
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(LoggerProvider, "add_log_record_processor"):
                with mock.patch.object(
                    ...
                    "__new__",
                    return_value=mock_batch_log_record_processor,
                ):
                    logging_util_inst.setup_logging(force_initialise)
                    BatchLogRecordProcessor.__new__.assert_called_once_with(
                        BatchLogRecordProcessor,
                        mock_exporter,
                        schedule_delay_millis=5000,
                    )
                    LoggerProvider.add_log_record_processor.assert_called_once_with(
...
def test_logging_util__logging_to_appins__with_args():
    ...
    with mock.patch.object(LoggingUtil, "log_info", return_value=None):
        with mock.patch.object(LoggingUtil, "_initialise", return_value=None):
            ...
            LoggingUtil.log_info.assert_called_once_with(
                f"Function my_function_with_args called with args: {', '.join(args_repr + kwargs_repr)}"
            )
            assert resp == "Hello world (1, 2, bob)"


def test_logging_util__logging_to_appins__with_notebook_exception():
    notebook_exit_exception = (
        notebookutils.mssparkutils.handlers.notebookHandler.NotebookExit(
            "Some exception"
        )
    )
    ...
    with mock.patch.object(LoggingUtil, "log_info", return_value=None):
        with mock.patch.object(LoggingUtil, "_initialise", return_value=None):
            with mock.patch.object(
                notebookutils.mssparkutils.notebook.exit,
                "exit",
                return_value="notebook exit",
            ):
                ...
                LoggingUtil.log_info.assert_has_calls(
                    [
                        mock.call(
                            f"Function my_function_with_notebook_exception called with args: {', '.join(args_repr + kwargs_repr)}"
                        ),
                        mock.call("Notebook exited: Some exception"),
                    ]
                )
                notebookutils.mssparkutils.notebook.exit.assert_called_once_with(
                    notebook_exit_exception
                )
