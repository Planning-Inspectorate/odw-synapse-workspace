...
from odw.core.etl.transformation.curated.checkmark_comments_curated_process import (
...
_MODULE = "odw.core.etl.transformation.curated.checkmark_comments_curated_process"
...
def _run(spark, df_in):
    ...
    with (
        ...
        inst = CheckmarkCommentsCuratedProcess(spark)
...
class TestCheckmarkCommentsCuratedProcessIntegration(ETLTestCase):

    def test__comments_curated__run__passes_data_through_unchanged(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [_SAMPLE_ROW])

        data_to_write, _ = _run(spark, df_in)
        df_out = data_to_write["odw_curated_db.pbi_comments"]["data"]

        assert df_out.columns == _SAMPLE_COLUMNS
        assert df_out.collect()[0] == df_in.collect()[0]

    def test__comments_curated__run__write_metadata_targets_curated_layer(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [_SAMPLE_ROW])

        data_to_write, _ = _run(spark, df_in)
        write_config = data_to_write["odw_curated_db.pbi_comments"]
        ...
        assert write_config["table_name"] == "pbi_comments"
        assert write_config["database_name"] == "odw_curated_db"
        assert write_config["blob_path"] == "checkmarkdata/pbi_comments"

    def test__comments_curated__run__output_table_path_is_correct(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [_SAMPLE_ROW])
        ...
        assert out_keys == ["odw_curated_db.pbi_comments"]

    def test__comments_curated__run__multiple_rows_pass_through_with_correct_count(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        ...
        assert result.metadata.activity_type == "CheckmarkCommentsCuratedProcess"

    def test__comments_curated__run__empty_source_writes_empty_output(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [])
        ...
        assert data_to_write["odw_curated_db.pbi_comments"]["write_mode"] == "overwrite"
