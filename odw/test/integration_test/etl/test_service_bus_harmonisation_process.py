...
class TestServiceBusHarmonisationProcess(ETLTestCase):
    ...
    def generate_harmonised_table_schema(
        self, base_schema: T.StructType, incremental_key_col: str
    ):
        # Note: delta tables do not support non-nullable columns
        return T.StructType(
            base_schema.fields
    ...
    def test__service_bus_harmonisation_process__run__with_existing_data_same_schema(
        self,
    ):
        """
        ...
        existing_data_ingestion_date = datetime.strptime(
        ...
        standardised_data = spark.createDataFrame(
            (
                ("a", "b", "c", "id1")
                + already_existing_standardised_col_data,  # Already in the harmonised table
                ("d", "e", "f", "id2")
                + already_existing_standardised_col_data,  # Already in the harmonised table
                ("e", "f", "g", "id3")
                + already_existing_standardised_col_data,  # Already in the harmonised table
                ("p", "q", "r", "id4")
                + already_existing_standardised_col_data,  # Already in the harmonised table (but will be updated)
                ("x", "y", "z", "id5")
                + already_existing_standardised_col_data,  # Already in the harmonised table (but will be deleted)
                ("g", "h", "i", "id6")
                + created_standardised_col_data,  # New data to add to the harmonised table
                ("j", "k", "l", "id7")
                + created_standardised_col_data,  # New data to add to the harmonised table
                ("m", "n", "o", "id8")
                + created_standardised_col_data,  # New data to add to the harmonised table
                ("p", "q", "s", "id9")
                + updated_standardised_col_data,  # A column to be updated, "col_c" updated from 'r' -> 's'
                ("x", "y", "z", "id10")
                + deleted_standardised_col_data,  # A column to be deleted
            ),
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(Util, "display_dataframe"):
                ...
                actual_table_data = spark.table(
                    "odw_harmonised_db.test_sb_hrm_pc_exst_data"
                )
    ...
    def test__service_bus_harmonisation_process__run__with_existing_data_different_schema(
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(Util, "display_dataframe"):
                ...
                assert_etl_result_successful(result)
                actual_table_data = spark.table(
                    "odw_harmonised_db.test_sb_hrm_pc_chg_schema"
                )
                self.compare_harmonised_data(
                    expected_harmonised_data_after_writing, actual_table_data
                )
    ...
    def test__service_bus_harmonisation_process__run__with_no_existing_data(self):
        ...
        with mock.patch.object(
            ...
            with mock.patch.object(Util, "display_dataframe"):
                ...
                assert_etl_result_successful(result)
                actual_table_data = spark.table(
                    "odw_harmonised_db.test_sb_hrm_pc_no_data"
                )
                self.compare_harmonised_data(
                    expected_harmonised_data_after_writing, actual_table_data
                )
