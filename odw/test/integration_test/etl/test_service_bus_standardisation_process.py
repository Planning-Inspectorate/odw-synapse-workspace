...
class TestServiceBusStandardisationProcess(ETLTestCase):
    ...
    def test__service_bus_standardisation_process__run__with_existing_data(self):
        ...
        test_data = spark.createDataFrame(
            [
                (
                    1,
                    "a",
                    f"file://{Path().resolve()}/odw-raw/ServiceBus/{entity_name}/myfile1.json",
                ...
                (
                    3,
                    "c",
                    f"file://{Path().resolve()}/odw-raw/ServiceBus/{entity_name}/myfile2.json",
                    message_enqueued_time_utc,
                ),
            ],
        ...
        for input_file, content in input_files.items():
            file_to_create = (
                f"{PytestSparkSessionUtil().get_spark_warehouse_name()}/{input_file}"
            )
        ...
        extra_cols_added_during_processing = [
            "expected_from",
            "expected_to",
            "ingested_datetime",
            "input_file",
        ...
        with mock.patch.object(
            SchemaUtil, "get_service_bus_schema", return_value=mock_standardised_schema
        ):
        ...
        data_after = spark.table(
            "odw_standardised_db.sb_test__service_bus_standardisation_process__run"
        )
