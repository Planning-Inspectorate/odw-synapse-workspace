...
def _run(spark, df_in):
    ...
    with (
        ...
        mock.patch("odw.core.etl.transformation.harmonised.checkmark_comment_type_ref_harmonisation_process.LoggingUtil") as mock_proc_logging,
    ):
...
class TestCheckmarkCommentTypeRefHarmonisationProcessIntegration(ETLTestCase):

    def test__comment_type_ref_harmonisation__run__writes_single_row_with_rowid_and_ingestion_date(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [_SAMPLE_ROW])

        data_to_write, result = _run(spark, df_in)

        write_config = data_to_write['odw_harmonised_db.comment_type_ref']
        ...
        assert write_config["table_name"] == 'comment_type_ref'
        assert write_config["database_name"] == "odw_harmonised_db"
        assert write_config["blob_path"] == 'comment_type_ref'
    ...
    def test__comment_type_ref_harmonisation__run__rowid_is_deterministic_for_same_input(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        ...
        rowid_a = out_a['odw_harmonised_db.comment_type_ref']["data"].collect()[0]["RowID"]
        rowid_b = out_b['odw_harmonised_db.comment_type_ref']["data"].collect()[0]["RowID"]
        assert rowid_a == rowid_b

    def test__comment_type_ref_harmonisation__run__rowid_changes_when_any_column_changes(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_a = _df(spark, [_SAMPLE_ROW])
    ...
    def test__comment_type_ref_harmonisation__run__truncate_load_overwrites_with_multiple_rows(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [_SAMPLE_ROW, _VARIANT_ROW])

        data_to_write, result = _run(spark, df_in)
        df_out = data_to_write['odw_harmonised_db.comment_type_ref']["data"]
    ...
    def test__comment_type_ref_harmonisation__run__handles_null_columns_without_breaking_rowid(self):
        spark = PytestSparkSessionUtil().get_spark_session()
    ...
    def test__comment_type_ref_harmonisation__run__empty_source_writes_empty_output(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [])
        ...
        assert data_to_write['odw_harmonised_db.comment_type_ref']["write_mode"] == "overwrite"
