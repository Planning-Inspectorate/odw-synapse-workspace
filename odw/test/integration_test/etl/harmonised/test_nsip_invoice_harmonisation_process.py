...
class TestNsipInvoiceHarmonisationProcess(ETLTestCase):
    ...
    def test__nsip_invoice_harmonisation_process__run__initial_load_matches_legacy(
        ...
        self.write_source_table(spark, spark.createDataFrame(source_rows, _standardised_schema()), test_case)
        ...
        with (
            mock.patch.object(F, "current_timestamp", return_value=F.lit(MOCK_TIMESTAMP)),
            mock.patch.object(NsipInvoiceHarmonisationProcess, "SERVICE_BUS_TABLE", f"{test_case}_sb_nsip_project"),
    ...
    def test__nsip_invoice_harmonisation_process__run__starts_surrogate_key_from_existing_max_id_plus_one(
        ...
        self.write_source_table(spark, spark.createDataFrame(source_rows, _standardised_schema()), test_case)
        self.write_target_table(spark, spark.createDataFrame(existing_rows, _harmonised_schema()), test_case)
    ...
    def test__nsip_invoice_harmonisation_process__run__empty_source_returns_empty_output(
        ...
        self.write_source_table(spark, spark.createDataFrame([], _standardised_schema()), test_case)
        self.write_empty_target_table(spark, test_case)
