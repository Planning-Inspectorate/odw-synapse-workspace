...
def _run(spark, df_in):
    ...
    with (
        ...
        mock.patch("odw.core.etl.transformation.harmonised.checkmark_outcome_reference_harmonisation_process.LoggingUtil") as mock_proc_logging,
    ):
        mock_etl_logging.return_value = mock.Mock()
        mock_proc_logging.return_value = mock.Mock()

        inst = CheckmarkOutcomeReferenceHarmonisationProcess(spark)
...
class TestCheckmarkOutcomeReferenceHarmonisationProcessIntegration(ETLTestCase):

    def test__outcome_reference_harmonisation__run__writes_single_row_with_rowid_and_ingestion_date(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [_SAMPLE_ROW])

        data_to_write, result = _run(spark, df_in)

        write_config = data_to_write['odw_harmonised_db.outcome_reference']
        ...
        assert write_config["table_name"] == 'outcome_reference'
        assert write_config["database_name"] == "odw_harmonised_db"
        assert write_config["blob_path"] == 'outcome_reference'

        assert result.metadata.insert_count == 1
        assert result.metadata.update_count == 0
        assert result.metadata.delete_count == 0
        assert result.metadata.activity_type == "CheckmarkOutcomeReferenceHarmonisationProcess"

    def test__outcome_reference_harmonisation__run__rowid_is_deterministic_for_same_input(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        ...
        rowid_a = out_a['odw_harmonised_db.outcome_reference']["data"].collect()[0]["RowID"]
        rowid_b = out_b['odw_harmonised_db.outcome_reference']["data"].collect()[0]["RowID"]
        assert rowid_a == rowid_b

    def test__outcome_reference_harmonisation__run__rowid_changes_when_any_column_changes(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_a = _df(spark, [_SAMPLE_ROW])
    ...
    def test__outcome_reference_harmonisation__run__truncate_load_overwrites_with_multiple_rows(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [_SAMPLE_ROW, _VARIANT_ROW])

        data_to_write, result = _run(spark, df_in)
        df_out = data_to_write['odw_harmonised_db.outcome_reference']["data"]
    ...
    def test__outcome_reference_harmonisation__run__handles_null_columns_without_breaking_rowid(self):
        spark = PytestSparkSessionUtil().get_spark_session()
    ...
    def test__outcome_reference_harmonisation__run__empty_source_writes_empty_output(self):
        spark = PytestSparkSessionUtil().get_spark_session()
        df_in = _df(spark, [])
        ...
        assert data_to_write['odw_harmonised_db.outcome_reference']["write_mode"] == "overwrite"
