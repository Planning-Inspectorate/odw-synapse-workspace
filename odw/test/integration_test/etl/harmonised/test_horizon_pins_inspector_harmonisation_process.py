...
class TestHorizonPinsInspectorHarmonisationProcess(ETLTestCase):
    ...
    def _run(self, spark, test_case):
        with (
            ...
            mock.patch.object(
                HorizonPinsInspectorHarmonisationProcess,
                "STAGE_TABLE",
                f"odw_harmonised_db.{self.test_case}_pins_inspector_stg",
            ),
            mock.patch.object(
                HorizonPinsInspectorHarmonisationProcess,
                "OUTPUT_TABLE",
                f"odw_harmonised_db.{self.test_case}_horizon_pins_inspector",
            ),
        ):
    ...
    def test__run__builds_scd2_timeline_end_to_end(self):
        ...
        actual_df = actual.select(
            ...
            "IsActive",
            "ODTSourceSystem",
        ).orderBy("horizonId", "IngestionDate")
