...
_OUTCOME_REFERENCE_COLUMNS = [
    "ID",
    "value",
]


class CheckmarkOutcomeReferenceHarmonisationProcess(HarmonisationProcess):
    """
    Harmonisation process for Checkmark outcome_reference data.
    ...
    SOURCE_TABLE = "odw_standardised_db.outcomereference"
    OUTPUT_TABLE = "odw_harmonised_db.outcome_reference"
    ...
    def get_name(cls) -> str:
        return "checkmark-outcome-reference-harmonised"
    ...
    def process(self, **kwargs) -> Tuple[Dict[str, DataFrame], ETLResult]:
        ...
        df = df.select(*_OUTCOME_REFERENCE_COLUMNS)

        # Reference tables use CURRENT_DATE() in legacy notebooks, not CURRENT_TIMESTAMP()
        df = df.withColumn("IngestionDate", F.current_date())

        hash_inputs = [
            F.coalesce(F.trim(F.col(c).cast("string")), F.lit(""))
            for c in _OUTCOME_REFERENCE_COLUMNS
        ]
        ...
        data_to_write = {
            self.OUTPUT_TABLE: {
                ...
                "table_name": "outcome_reference",
                "storage_endpoint": Util.get_storage_account(),
                "container_name": "odw-harmonised",
                "blob_path": "outcome_reference",
                "file_format": "delta",
