...
class DependencyResolver:
    def __init__(self, config: Dict[str, Any]):
        ...
        if not isinstance(config, dict):
            raise ValueError(
                f"Expected the given config to be a dictionary, but was of type {type(config)}"
            )
    ...
    def _preprocess_entity_stages(self, entity_stages: List[str]):
        ...
        invalid_entries = [
            str(entry)
            for entry in entity_stages
            if len(str(entry).split(delimiter)) > 2
        ]
        if invalid_entries:
            raise ValueError(
        ...
        valid_entries = [
            entry for entry in entity_stages if len(entry.split(delimiter)) == 2
        ]
        # Get the entries that do not have a delimiter - element after the delimiter can be inferred from the config
        entities_to_explode = [
            entry for entry in entity_stages if len(entry.split(delimiter)) == 1
        ]
        missing_entities = [
            entity
            for entity in entities_to_explode
            if entity not in self.config.get("entities", dict())
        ]
        if missing_entities:
            raise ValueError(
        ...
        exploded_entries = [
            [
                f"{entity}.{stage}"
                for stage in self.config.get("entities", dict())
                .get(entity, dict())
                .keys()
            ]
            for entity in entities_to_explode
        ...
        missing_entity_stages = [
            ...
            if not self.config.get("entities", dict())
            .get(entity_stage.split(delimiter)[0], dict())
            .get(entity_stage.split(delimiter)[1], None)
        ]
        if missing_entity_stages:
            raise ValueError(
                f"The following entity_stages could not be found in the config: {json.dumps(missing_entity_stages, indent=4)}"
            )
        return entries
    ...
    def _preprocess_config(cls, config: Dict[str, Any]):
        ...
        """
        entities: Dict[str, Dict[str, Dict[str, Union[Any, List[Any]]]]] = config.get(
            "entities", dict()
        )
    ...
    def _topological_sort(cls, config: Dict[str, Any]):
        ...
        entity_stages = {
            f"{entity_name}.{stage_name}": stage
            for entity_name, entity in entities.items()
            for stage_name, stage in entity.items()
        }
        dependency_map = {
            orchestration_entity_stage_name: tuple(entity["depends_on"])
            for orchestration_entity_stage_name, entity in entity_stages.items()
        }
        ...
        return [
            [
                entity_stages[orchestration_entity_stage_name]
                for orchestration_entity_stage_name in group
            ]
            for group in ordered_groups
        ]

    def _filter_irrelevant_dependencies_from_config(
        self, entity_stages_to_keep: List[str] = []
    ):
        """
        ...
        if not entity_stages_to_keep:
            ...
            return
        entities: Dict[str, Dict[str, Dict[str, Union[Any, List[Any]]]]] = (
            self.config.get("entities", dict())
        )
        entity_stages = {
            f"{entity_name}.{stage_name}": stage
            for entity_name, entity in entities.items()
    ...
    def filter_already_executed_entity_stages(
        self,
        topological_order_groups: List[List[Dict[str, Any]]],
        ...
        successful_executions = {
            f"{row['entity_name']}.{row['stage_name']}"
        ...
        cleaned_groups = [
            [
                entity
                for entity in group
                if entity["orchestration_entity_stage_name"]
                not in successful_executions
            ]
    ...
    @classmethod
    def filter_entity_stages_with_failed_dependencies(
        cls, group: List[Dict[str, Any]], execution_details: List[Dict[str, Any]]
    ):
        """
        ...
        return [
            entity_stage
            for entity_stage in group
            if not any(
                dependency in failed_executions
                for dependency in entity_stage.get("depends_on", [])
            )
        ]

    def generate_stages_to_run(
        self, entity_stages: List[str], execution_details: Dict[str, Any]
    ):
        """
