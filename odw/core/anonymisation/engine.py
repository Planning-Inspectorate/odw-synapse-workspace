...
class AnonymisationEngine:
    ...
    def get_cols_to_anonymise(self, entity_name: str, source_folder: Literal["ServiceBus", "Horizon", "entraid"]) -> Dict[str, Tuple[str, BaseStrategy]]:
        """
        Return a dictionary with the structure <col_name: (col_type, anonymisation_strategy)>

        :param entity_name str: The name of the entity
        :param source_folder str: The folder the entity is part of. Must be one of `ServiceBus`, `Horizon`, or `entraid`
        """
        storage_host = Util.get_storage_account()
        # ServiceBus, Horizon, entraid
        asset_qualified_name = _build_asset_qualified_name_from_params(
            storage_host=storage_host,
            source_folder=source_folder,
            entity_name=entity_name,
        ...
        for col in cols:
            ...
            classification_classes = set(col.get("classifications", []))
            for strat in self.strategies:
                if classification_classes.intersection(strat.classification_names):
                    cols_to_keep[col_name] = (col_type, strat)
                    break
        return cols_to_keep
