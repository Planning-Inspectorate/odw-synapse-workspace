...
class BaseStrategy(ABC):
    ...
    def mask_keep_first_last(col: Column) -> Column:
        ...
        return F.when(col.isNull(), None).otherwise(
            F.regexp_replace(col.cast("string"), r"(?<=.).(?=.)", "*")
        )
    ...
    @staticmethod
    def random_date_from_seed(
        seed: Column, start: str = "1955-01-01", end: str = "2005-12-31"
    ) -> Column:
...
class NINumberStrategy(BaseStrategy):
    ...
    def apply(
        ...
        first = F.element_at(
            letters_arr,
            (F.abs(F.hash(F.concat(seed, F.lit("_ni1")))) % 24 + 1).cast("int"),
        )
        second = F.element_at(
            letters_arr,
            (F.abs(F.hash(F.concat(seed, F.lit("_ni2")))) % 24 + 1).cast("int"),
        )
        digits = F.lpad(
            (F.abs(F.hash(F.concat(seed, F.lit("_nid")))) % 1000000).cast("string"),
            6,
            "0",
        )
        last = F.element_at(
            last_arr, (F.abs(F.hash(F.concat(seed, F.lit("_nil")))) % 4 + 1).cast("int")
        )
        ni_num = F.concat(first, second, digits, last)
        return df.withColumn(
            column, F.when(F.col(column).isNull(), None).otherwise(ni_num)
        )
...
class NameMaskStrategy(BaseStrategy):
    ...
    def apply(
        ...
        return df.withColumn(
            column, F.when(F.col(column).isNull(), None).otherwise(F.lit("REDACTED"))
        )
...
class AgeStrategy(BaseStrategy):
    ...
    def apply(
        ...
        return df.withColumn(
            column, BaseStrategy.random_int_from_seed(seed, 18, 70).cast("int")
        )
    ...
    def apply(
        ...
        if col_type and col_type.startswith("struct"):
            ...
            if isinstance(schema, StructType):
                ...
                for field in schema.fields:
                    if field.name.lower() == "postcode":
                        ...
                        redacted_fields.append(
                            F.when(pc.isNull(), None)
                            .otherwise(
                                F.split(F.trim(pc.cast("string")), r"\s+").getItem(0)
                            )
                            .alias(field.name)
                        )
                    else:
                ...
                return df.withColumn(
                    column,
                    F.when(F.col(f"`{column}`").isNotNull(), redacted_struct).otherwise(
                        None
                    ),
                )
        ...
        return df.withColumn(
            column,
            F.when(F.col(f"`{column}`").isNotNull(), F.lit("REDACTED")).otherwise(None),
        )
