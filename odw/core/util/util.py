...
class Util:
    ...
    def is_non_production_environment(cls) -> bool:
        """
        Return True if the current environment is DEV, TEST, or BUILD.
        """
        env = cls.get_environment().lower()
        return env in ("dev", "test", "build")
    ...
    def get_storage_account(cls) -> str:
        ...
        """
        connection_string = mssparkutils.credentials.getFullConnectionString(
            "ls_storage"
        )
    ...
    def get_path_to_file(cls, path: str):
        ...
        if not len(path_split) == 2:
            raise ValueError(
                f"Path should have the format 'container_name/path' but was '{path}'"
            )
