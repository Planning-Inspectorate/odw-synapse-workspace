...
class SynapseDeltaIO(SynapseDataIO):
    ...
    def write(self, data: DataFrame, **kwargs):
        ...
        :param str primary_keys: A list of primary keys in the data
        ...
        if not (all(x in delta_table_cols for x in primary_keys) and all(x in new_data_cols for x in primary_keys)):
            missing_in_delta = [x for x in primary_keys if x not in delta_table_cols]
            missing_in_new_data = [x for x in primary_keys if x not in new_data_cols]
            raise ValueError(
                f"Not all specified primary_keys are in the target or new data.\n"
                f"Of the following primary keys {primary_keys}\n"
        ...
        target_delta_table.alias("t").merge(
            data.alias("s"),
            " & ".join(f"t.{key} = s.{key}" for key in primary_keys)
